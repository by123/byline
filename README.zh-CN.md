...
- **选中即复制 & 即时翻译** —— 选中文本自动复制到剪贴板；右键 → **翻译**，译文就在
  点击处弹出（Google 翻译，通常一秒内出结果；×、Esc 或点击其他区域即可关闭）。
  翻译方向在偏好设置中有独立的「翻译」模块 —— 23 种界面语言任选，默认 英文 → 简体中文
...
- **会话快捷指令** —— 右键侧栏中的会话，把保存好的提示词（"继续"、"提交改动"…）
  直接发进那个会话，无需切换标签
- **终端区域右键菜单** —— 复制/粘贴/翻译，加上同样的快捷指令、移交操作和 YOLO 启动项，
  就在你工作的地方
...
├── renderer/
│   ├── index.html     xterm.js 界面：标签、侧栏、文件树、状态机、命令面板
